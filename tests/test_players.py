...
from gomoku_ai.ai import AlphaBetaV5AI
...
def test_available_algorithms_include_baseline_and_alphabeta():
    assert available_algorithms() == ("v0", "v1", "v2", "v3", "v4", "v5")
...
def test_player_spec_defaults_to_latest_depth_five():
    spec = PlayerSpec()

    assert spec.normalized_algorithm == "v5"
    assert spec.depth == 5
...
def test_create_player_accepts_v5_alias():
    player = create_player(BLACK, PlayerSpec("v5", depth=1))

    assert isinstance(player, AlphaBetaV5AI)
    assert player.name == "v5"
