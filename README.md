...
- `alpha-beta:v4`：第四版 alpha-beta，也是当前默认算法，在第三版棋型和战术保留基础上加入候选分析复用、线型分析复用和字符串窗口评分，减少重复扫描。
...
- `alpha-beta:v4`：第四版。在 `v3` 基础上加入本轮性能优化：同一候选点只做一次综合分析，线型分析同时返回评分和威胁分类，并用字符串窗口评分减少列表切片与重复计数。
